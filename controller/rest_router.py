...
class OfCtl_v1_0(OfCtl):
    ...
    def set_flow(self, cookie, priority, dl_type=0, dl_dst=0, dl_vlan=0,
                 nw_src=0, src_mask=32, nw_dst=0, dst_mask=32,
                 nw_proto=0, idle_timeout=0, actions=None, table_id=1, instruction=None):
        ofp = self.dp.ofproto
        ofp_parser = self.dp.ofproto_parser
        cmd = ofp.OFPFC_ADD
    ...
    def set_routing_flow(self, cookie, priority, outport, dl_vlan=0,
        ...
        if outport is not None:
            instruction = ofp_parser.OFPInstructionGotoTable(qoslib.MANGLE_OUTPUT_TABLE_ID)
        ...
        self.set_flow(cookie, priority, dl_type=dl_type, dl_vlan=dl_vlan,
                      ...
                      idle_timeout=idle_timeout, actions=actions,instructions=instruction)
        ...
        self.set_flow(cookie, priority, dl_type=dl_type, dl_vlan=dl_vlan,
                      nw_src=nw_src, src_mask=src_mask,
                      nw_dst=nw_dst, dst_mask=dst_mask,
                      idle_timeout=idle_timeout, actions=actions, table_id=qoslib.MANGLE_POST_ROUTING_TABLE_ID)
    ...
    def delete_flow(self, flow_stats):
        ...
        flow_mod = self.dp.ofproto_parser.OFPFlowMod(
            self.dp, match, cookie, 1, cmd, priority=priority, actions=actions)
...
class OfCtl_after_v1_2(OfCtl):
    ...
    def set_routing_flow(self, cookie, priority, outport, dl_vlan=0,
        ...
        self.set_flow(cookie, priority, dl_type=dl_type, dl_vlan=dl_vlan,
                      nw_src=nw_src, src_mask=src_mask,
                      nw_dst=nw_dst, dst_mask=dst_mask,
                      idle_timeout=idle_timeout, actions=actions, instruction=instruction)
